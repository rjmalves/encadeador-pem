...
class ProcessadorDecomp:
    ...
    def gt_percentual(relato: Relato,
        ...
        def extrai_gts_semana(relato: Relato,
            ...
            gt = relato.geracao_termica_subsistema[["Subsistema",
                                                    f"Estágio {semana}"]]
            gt = gt.set_index("Subsistema")
            gt = gt.drop(index="FC")
            return gt

        def extrai_gts_min_max_semana(relato: Relato,
                                      relgnl: RelGNL,
                                      semana: int) -> pd.DataFrame:
            ...
            term_1s = pd.concat([term_1s, term_1s_gnl],
                                ignore_index=True)
            term_1s
            ...
            for idx, lin in term_1s.iterrows():
                ...
                taxa_disp = float(disp.loc[filtro,
                                           f"Estágio {semana}"]) / 100.0
                for pat in [1, 2, 3]:
