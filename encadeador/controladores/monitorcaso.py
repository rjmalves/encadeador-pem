    ...
    def executa_caso(self) -> bool:
        try:
            ...
            self._gerenciador.agenda_job(self._caminho_job,
                                         self._nome_job,
                                         self.caso.numero_processadores)
            armazenador = ArmazenadorCaso(self.caso, self._log)
            ...
            if ultimo_estado == EstadoJob.EXECUTANDO:
                sintetizador = SintetizadorCasoNEWAVE(self.caso, self._log)
...
class MonitorDECOMP(MonitorCaso):
    ...
    def executa_caso(self) -> bool:
        try:
            ...
            if ultimo_estado == EstadoJob.EXECUTANDO:
                sintetizador = SintetizadorCasoDECOMP(self.caso, self._log)
                if not sintetizador.sintetiza_caso():
                    return False
                self.caso.finaliza_caso(True)
                self._log.info("Caso concluído com sucesso")
