...
class AvaliadorDECOMP(AvaliadorCaso):
    ...
    PADRAO_MAX_ITER = "CONVERGENCIA NAO ALCANCADA EM"
    ...
    def __avalia_maximo_iteracoes(self) -> bool:
        try:
            with self._uow:
                relato = self._uow.decomp.get_relato()
                return any(
                    [
                        AvaliadorDECOMP.PADRAO_MAX_ITER in b.data
                        for b in relato.data.of_type(DefaultBlock)
                    ]
                )
        except Exception as e:
    ...
    def avalia(self) -> TransicaoCaso:
        ...
        if self.__avalia_erro_dados():
            return TransicaoCaso.ERRO_DADOS
        if self.__avalia_gap_negativo():
            return TransicaoCaso.ERRO_CONVERGENCIA
        if self.__avalia_maximo_iteracoes():
            return TransicaoCaso.NAO_CONVERGIU
