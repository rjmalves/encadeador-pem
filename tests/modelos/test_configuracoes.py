...
def test_configuracoes_validas():
    load_dotenv(join(DIRETORIO_TESTE, "valido.cfg"), override=True)
...
def test_nome_estudo_invalido():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_nome_estudo.cfg"), override=True
        )
...
def test_arquivo_lista_casos_inexistente():
    with pytest.raises(FileNotFoundError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_arquivo_lista_casos.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_nome_diretorio_newave_invalido():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_nome_diretorio_newave.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_nome_diretorio_decomp_invalido():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_nome_diretorio_decomp.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_gerenciador_fila():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_gerenciador_fila.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_versoes():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_versoes.cfg"), override=True
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_flexibiliza_deficit():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_flex_deficit.cfg"), override=True
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_maximo_flebilizacoes_revisao():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_maximo_flexibilizacoes.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_ultimas_iteracoes_flexibilizacao():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_ultimas_iteracoes.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_metodo_flexibilizacao():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_metodo_flexibilizacao.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_adequa_decks_newave():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_adequa_decks_newave.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_cvar():
    with pytest.raises(ValueError):
        load_dotenv(join(DIRETORIO_TESTE, "invalido_cvar.cfg"), override=True)
        Configuracoes.le_variaveis_ambiente()
...
def test_opcao_parpa():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_opcao_parpa.cfg"), override=True
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_adequa_decks_decomp():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_adequa_decks_decomp.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_previne_gap_negativo():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_previne_gap_negativo.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_maximo_iteracoes_decomp():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_maximo_iteracoes_decomp.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_fator_aumento_gap():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_fator_aumento_gap.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_gap_maximo():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_gap_maximo.cfg"), override=True
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_diretorio_instalacao():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_diretorio_instalacao.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_processadores():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_processadores.cfg"), override=True
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_ajuste_processadores():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_ajuste_processadores.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_variaveis_encadeadas():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_variaveis_encadeadas.cfg"),
            override=True,
        )
        Configuracoes.le_variaveis_ambiente()
...
def test_script_converte_codificacao():
    with pytest.raises(ValueError):
        load_dotenv(
            join(DIRETORIO_TESTE, "invalido_script_converte.cfg"), override=True
        )
        Configuracoes.le_variaveis_ambiente()
